...
void calc_ApEn(int, int, double, int, double[], double[]);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	/* Prepare output data. A pointer points to the location of ApEn */
    plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
    ApEn = mxGetPr(plhs[0]);

    /* Call the ApEn subroutine */
    calc_ApEn(m, tau, r, N, u, ApEn);

}


void calc_ApEn(int m, int tau, double r, int N, double *u, double *ApEn){
    *ApEn = calc_Phi(u, N, r, m, tau) - calc_Phi(u, N, r, (m+1), tau);
