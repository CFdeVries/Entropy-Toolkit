...
void calc_SampEn(int, int, double, int, double[], double[]);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	/* Prepare output data. A pointer points to the location of SampEn */
    plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
    SampEn = mxGetPr(plhs[0]);

    /* Call the SampEn subroutine */
    calc_SampEn(m, tau, r, N, u, SampEn);
}


void calc_SampEn(int m, int tau, double r, int N, double *u, double *SampEn){
    *SampEn = -log((calc_matches(u, N, r, m+1, tau, m))/(calc_matches(u, N, r, m, tau, m)));
}
